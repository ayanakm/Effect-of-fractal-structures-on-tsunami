    ...
    for (int k = 0; k < 6; k++) {
        ...
        for (int i = 1; i <= S; i++) {
            ...
            for (int j = 3; j <= N; j++) {
                ...
                dif_u[j] = old_u[j-1] - old_u[j+1] + (tau / alpha) * ((3.0 / 2.0) * (u[j+1] - u[j-1]) * (u[j+1] - u[j-1]) + 6.0 * u[j] * (u[j+1] - 2.0 * u[j] + u[j-1])) + (1.0 / alpha * alpha) * ((u[j+2] - 4.0 * u[j+1] + 6.0 * u[j] - 4.0 * u[j-1] + u[j-2])) + 3.0 * (alpha * tau /(beta * beta)) * (plus_u[j] - 2.0 * u[j] + minus_u[j]);
                //(-1) * dif_uを代入
